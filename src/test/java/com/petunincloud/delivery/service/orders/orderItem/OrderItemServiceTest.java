...
class OrderItemServiceTest {
    ...
    void addItemToOrder_ShouldAddItemAndUpdateTotalPrice() {
        ...
        when(orderItemMapper.toEntity(request, order, dishResponse))
                .thenReturn(entity);
        when(orderItemRepository.save(entity))
                .thenReturn(savedEntity);
        when(orderItemMapper.toResponse(savedEntity))
                .thenReturn(response);
    ...
    void getOrderItems_ShouldGetOrderItems() {
        ...
        when(orderRepository.findByIdWithItems(orderId))
                .thenReturn(Optional.of(order));
        when(orderItemMapper.toResponse(item1))
                .thenReturn(response1);
        when(orderItemMapper.toResponse(item2))
        ...
        verify(orderItemMapper, times(2))
                .toResponse(any(OrderItemEntity.class));
    }
